...
void cpu_layout(layout_kernel::layout_config_t config, double *etas, double *zetas, layout_kernel::node_data_t &node_data, layout_kernel::path_data_t &path_data) {
    int nbr_threads = config.nthreads;
    std::cout << "CPU layout (" << nbr_threads << " threads)" << std::endl;
...
void layout_func(layout_config_t config, const odgi::graph_t &graph, std::vector<std::atomic<double>> &X, std::vector<std::atomic<double>> &Y) {
    ...
    layout_kernel::node_data_t node_data;
    node_data.node_count = node_count;
    node_data.nodes = (layout_kernel::node_t*) malloc(node_count * sizeof(layout_kernel::node_t));
    for (int node_idx = 0; node_idx < node_count; node_idx++) {
        layout_kernel::node_t *n_tmp = &node_data.nodes[node_idx];
    ...
    layout_kernel::path_data_t path_data;
    path_data.path_count = path_count;
    path_data.total_path_steps = 0;
    path_data.paths = (layout_kernel::path_t*) malloc(path_count * sizeof(layout_kernel::path_t));
...
#pragma omp parallel for num_threads(config.nthreads)
    ...
    for (int node_idx = 0; node_idx < node_count; node_idx++) {
        layout_kernel::node_t *n = &(node_data.nodes[node_idx]);
