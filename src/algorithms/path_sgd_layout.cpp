...
namespace odgi {
    namespace algorithms {
        ...
        // Internal struct for interleaved coordinates
        struct NodeCoords {
            std::atomic<double> x;
            std::atomic<double> y;
        };
        ...
        void path_linear_sgd_layout(const PathHandleGraph &graph,
            ...
            // Interleaved layout storage
            auto layout = std::make_unique<NodeCoords[]>(X.size());
            // Initialize from input X and Y
            #pragma omp parallel for schedule(static) num_threads(nthreads)
            for (size_t i = 0; i < X.size(); ++i) {
                layout[i].x.store(X[i].load(std::memory_order_relaxed), std::memory_order_relaxed);
                layout[i].y.store(Y[i].load(std::memory_order_relaxed), std::memory_order_relaxed);
            }
            ...
            if (at_least_one_path_with_more_than_one_step){
                        ...
                        [&]() {
                            while (work_todo.load(std::memory_order_relaxed)) {
                                if (term_updates.load(std::memory_order_relaxed) > min_term_updates) {
                                    ...
                                    if (iteration >= iter_max) {
                                    ...
                                    } else if (Delta_max.load(std::memory_order_relaxed) <= delta) { // nb: this will also break at 0
                                        if (progress) {
                                        ...
                                        eta.store(etas[iteration], std::memory_order_relaxed); // update our learning rate
                                        Delta_max.store(delta, std::memory_order_relaxed); // set our delta max to the threshold
                        ...
                        [&](uint64_t tid) {
                            ...
                            while (work_todo.load(std::memory_order_relaxed)) {
...
#endif
                                    double mu = eta.load(std::memory_order_relaxed) * w_ij;
                                    if (mu > 1) {
                                        mu = 1;
                                    }
                                ...
                                std::cerr << "nodes are " << graph.get_id(term_i) << " and " << graph.get_id(term_j) << std::endl;
                                    ...
                                    double dx = layout[2 * i + offset_i].x.load(std::memory_order_relaxed) - layout[2 * j + offset_j].x.load(std::memory_order_relaxed);
                                    double dy = layout[2 * i + offset_i].y.load(std::memory_order_relaxed) - layout[2 * j + offset_j].y.load(std::memory_order_relaxed);
                                    if (dx == 0) {
                                        dx = 1e-9; // avoid nan
                                    }
#ifdef debug_path_sgd
                                ...
                                std::cerr << "Delta_abs " << Delta_abs << std::endl;
                                    ...
                                    while (Delta_abs > Delta_max.load(std::memory_order_relaxed)) {
                                        Delta_max.store(Delta_abs, std::memory_order_relaxed);
                                    }
                                    // calculate update
                                    double r = Delta / mag;
                                    double r_x = r * dx;
...
#endif
                                    layout[2 * i + offset_i].x.store(layout[2 * i + offset_i].x.load(std::memory_order_relaxed) - r_x, std::memory_order_relaxed);
                                    layout[2 * i + offset_i].y.store(layout[2 * i + offset_i].y.load(std::memory_order_relaxed) - r_y, std::memory_order_relaxed);
                                    layout[2 * j + offset_j].x.store(layout[2 * j + offset_j].x.load(std::memory_order_relaxed) + r_x, std::memory_order_relaxed);
                                    layout[2 * j + offset_j].y.store(layout[2 * j + offset_j].y.load(std::memory_order_relaxed) + r_y, std::memory_order_relaxed);
#ifdef debug_path_sgd
                ...
                auto snapshot_lambda =
                        [&]() {
                            uint64_t iter = 0;
                            while (snapshot && work_todo.load(std::memory_order_relaxed)) {
                                if ((iter < iteration) && iteration != iter_max) {
                                    ...
                                    for (size_t k = 0; k < X.size(); ++k) {
                                        X_iter[k] = layout[k].x.load(std::memory_order_relaxed);
                                    }
                                    std::vector<double> Y_iter(Y.size());
                                    i = 0;
                                    for (size_t k = 0; k < Y.size(); ++k) {
                                        Y_iter[k] = layout[k].y.load(std::memory_order_relaxed);
                                    }
                ...
                // Copy back to X and Y
                #pragma omp parallel for schedule(static) num_threads(nthreads)
                for (size_t i = 0; i < X.size(); ++i) {
                    X[i].store(layout[i].x.load(std::memory_order_relaxed), std::memory_order_relaxed);
                    Y[i].store(layout[i].y.load(std::memory_order_relaxed), std::memory_order_relaxed);
                }
            }
