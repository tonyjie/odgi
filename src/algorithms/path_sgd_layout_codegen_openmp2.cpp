...
namespace odgi {
    namespace algorithms {
        ...
        void path_linear_sgd_layout(const PathHandleGraph &graph,
            ...
            uint64_t first_cooling_iteration = std::floor(cooling_start * static_cast<double>(iter_max));

            bool at_least_one_path_with_more_than_one_step = false;
            for (const auto& path : path_sgd_use_paths) {
            ...
            if (at_least_one_path_with_more_than_one_step) {
                ...
                double w_max = 1.0;
                auto etas = path_linear_sgd_layout_schedule(w_min, w_max, iter_max,
                                                            iter_with_max_learning_rate, eps);

                std::vector<double> zetas((space <= space_max ? space : space_max + 
                                        (space - space_max)/space_quantization_step + 1) + 1);
                double zeta_tmp = 0.0;
                ...
                const auto& np_bv = path_index.get_np_bv();
                const auto& nr_iv = path_index.get_nr_iv();
                const auto& npi_iv = path_index.get_npi_iv();
                ...
                #pragma omp parallel
                {
                    ...
                    while (work_todo.load(std::memory_order_relaxed)) {
                        const uint64_t step_index = dis_step(gen);
                        const uint64_t path_i = npi_iv[step_index];
                        const path_handle_t path = as_path_handle(path_i);

                        const size_t path_step_count = path_index.get_path_step_count(path);
                        if (path_step_count == 1) continue;

                        step_handle_t step_a, step_b;
                        as_integers(step_a)[0] = path_i;
                        const size_t s_rank = nr_iv[step_index] - 1;
                        ...
                        if (cooling.load(std::memory_order_relaxed) || flip(gen)) {
                            if ((s_rank > 0 && flip(gen)) || s_rank == path_step_count-1) {
                                const uint64_t jump_space = std::min(space, static_cast<uint64_t>(s_rank));
                                uint64_t space_idx = jump_space;
                                if (jump_space > space_max) {
                                    space_idx = space_max + (jump_space - space_max)/space_quantization_step + 1;
                                }
                                dirtyzipf::dirty_zipfian_int_distribution<uint64_t>::param_type z_p(
                                    1, jump_space, theta, zetas[space_idx]);
                                dirtyzipf::dirty_zipfian_int_distribution<uint64_t> z(z_p);
                                const uint64_t z_i = z(gen);
                                as_integers(step_b)[0] = as_integer(path);
                                as_integers(step_b)[1] = s_rank - z_i;
                            } else {
                                const uint64_t jump_space = std::min(space, static_cast<uint64_t>(path_step_count - s_rank - 1));
                                uint64_t space_idx = jump_space;
                                if (jump_space > space_max) {
                                    space_idx = space_max + (jump_space - space_max)/space_quantization_step + 1;
                                }
                        ...
                        const handle_t term_i = path_index.get_handle_of_step(step_a);
                        const handle_t term_j = path_index.get_handle_of_step(step_b);
                        const uint64_t term_i_length = graph.get_length(term_i);
                        const uint64_t term_j_length = graph.get_length(term_j);
                        ...
                        // const double term_dist = std::max(
                        //     std::abs(static_cast<double>(pos_in_path_a) - static_cast<double>(pos_in_path_b)),
                        //     1e-9);
                        ...
                        const double term_weight = 1.0 / term_dist;
                        const double mu = std::min(eta.load(std::memory_order_relaxed) * term_weight, 1.0);
                        const double d_ij = term_dist;

                        const uint64_t i = number_bool_packing::unpack_number(term_i);
                        const uint64_t j = number_bool_packing::unpack_number(term_j);
                        const uint64_t offset_i = use_other_end_a ? 1 : 0;
                        const uint64_t offset_j = use_other_end_b ? 1 : 0;
                        ...
                        // Added for generate code
                        if (dx == 0) {  
                            dx = 1e-9; // avoid nan
                        }
                        const double mag = std::sqrt(dx*dx + dy*dy);
                        const double Delta = mu * (mag - d_ij) / 2.0;

                        double current_max = Delta_max.load(std::memory_order_relaxed);
                        while (std::abs(Delta) > current_max && 
                            !Delta_max.compare_exchange_weak(current_max, std::abs(Delta),
                        ...
                        if (++term_updates_local >= 1000) {
                            ...
                            #pragma omp master
                            {
                                if (term_updates.load(std::memory_order_relaxed) >= min_term_updates) {
                                    const uint64_t current_iter = iteration.load(std::memory_order_relaxed);
        ...
        std::vector<double> path_linear_sgd_layout_schedule(const double &w_min,
            ...
            const double lambda = std::log(eta_max / eta_min) / (iter_max - 1);

            std::vector<double> etas;
            etas.reserve(iter_max + 1);
            for (int64_t t = 0; t <= iter_max; ++t) {
                etas.push_back(eta_max * std::exp(-lambda * std::abs(t - static_cast<int64_t>(iter_with_max_learning_rate))));
            }
            return etas;
        }
